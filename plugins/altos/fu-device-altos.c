...
static GString *
fu_device_altos_tty_read (FuDeviceAltos *device, guint timeout_ms, GError **error)
{
	FuDeviceAltosPrivate *priv = GET_PRIVATE (device);
	gint rc;
	struct pollfd fds;
...
fu_device_altos_read_page (FuDeviceAltos *device, guint address, GError **error)
{
	...
	if (!fu_device_altos_tty_write (device, cmd, -1, error))
		return NULL;
	str = fu_device_altos_tty_read (device, 500, error);
	if (str == NULL)
		return NULL;
	return g_steal_pointer (&str);
}
...
static void
fu_device_altos_version_parse (FuDeviceAltos *device, const gchar *str)
{
	FuDeviceAltosPrivate *priv = GET_PRIVATE (device);
	g_auto(GStrv) lines = g_strsplit_set (str, "\n\r", -1);
...
fu_device_altos_probe_bootloader (FuDeviceAltos *device, GError **error)
{
	...
	/* get the version infomation */
	if (!fu_device_altos_tty_write (device, "v\n", -1, error))
		return FALSE;
	str = fu_device_altos_tty_read (device, 100, error);
	if (str != NULL)
		fu_device_altos_version_parse (device, str->str);
